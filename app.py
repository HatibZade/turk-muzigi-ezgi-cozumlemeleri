...
with tab1:
    ...
    with col2:
        ...
        ns = makam.get("nazari_seyir", {})
        st.markdown(
            f"**Nazari Seyir**: Âgâz **{', '.join(ns.get('agaz', [])) or '—'}**, "
            f"Merkez **{', '.join(ns.get('kutb', [])) or '—'}**, "
            f"Karar **{', '.join(ns.get('karar', [])) or '—'}**"
        )
        ...
        ts = makam.get("lahni_seyir", {}).get("tasarruflar", [])
        if ts:
            st.markdown("**Lahnî Seyir Notları**")
            for t in ts:
                st.markdown(f"- {t}")
...
with tab2:
    ...
    st.info(
        "Bu bölümde yüklediğiniz nota üzerinden (ileride) karar perdesi, merkez ve seyir alanı çıkarılarak "
        "olası ezgi profilleri önerilecektir. Şimdilik sadece yükleme alanı aktiftir."
    )
